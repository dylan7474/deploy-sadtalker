...
- **S3 upload is NOT optional in the upstream repo**: `api.py` originally always
  ...
  `http://localhost:9090/results/<filename>.mp4`.
...
| Generated video vanishes / request errors with blank AWS creds | Upstream `api.py` always uploads to S3 and deletes the local file regardless of upload success | Script now patches `api.py` to serve locally when `AWS_S3_BUCKET_NAME` is unset |
