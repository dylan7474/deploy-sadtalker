...
# Swap to a stable mirror, add apt retry/timeout tolerance, disable HTTP
# pipelining (a transparent proxy/cache on the host network corrupts pipelined
# apt fetches - the symptom is cascading "File has unexpected size (A != B)"
# errors where each package gets the previous package's expected size), and make
# the install step tolerant of a single bad package fetch. Idempotent - safe to
# re-run.
...
RUN sed -i 's|http://archive.ubuntu.com/ubuntu|http://mirrors.kernel.org/ubuntu|g; s|http://security.ubuntu.com/ubuntu|http://mirrors.kernel.
org/ubuntu|g' /etc/apt/sources.list
...
if ( cd "$REPO_DIR" && python3 "$PATCH_PY" ); then
API_PATCH_OK=1
else
API_PATCH_OK=0
fi
...
echo "==> Verifying the container can actually initialise CUDA"
# The host-side 'nvidia-smi' passthrough test above is necessary but NOT
# sufficient: nvidia-smi talks to the driver without ever touching nvidia-uvm,
# so it passes even when CUDA itself is broken inside the container. The usual
# culprit is a stale CDI spec (/etc/cdi/nvidia.yaml) pinning an old major number
# for /dev/nvidia-uvm - nvidia-uvm gets a *dynamic* major that changes across
# module reloads/reboots, and containers then get a dead device node. Symptom:
# torch prints "CUDA unknown error ... Setting the available devices to be zero"
# and SadTalker silently runs on CPU (which has no working output path here).
if docker compose exec -T sadtalker python3 -c \
"import torch,sys; sys.exit(0 if torch.cuda.is_available() else 1)" 2>/dev/null; then
echo "  CUDA OK inside the container (torch.cuda.is_available() == True)."
else
echo "  WARNING: torch.cuda.is_available() is False inside the container."
echo "  nvidia-smi may still work in there - that is the classic stale-CDI symptom."
echo "  Fix on the host, then re-run this script:"
echo "    sudo nvidia-ctk cdi generate --output=/etc/cdi/nvidia.yaml"
echo "  (check: 'grep -A1 nvidia-uvm /etc/cdi/nvidia.yaml' must match the major in"
echo "   'grep nvidia-uvm /proc/devices'). Continuing, but /generate/ will not work."
fi
